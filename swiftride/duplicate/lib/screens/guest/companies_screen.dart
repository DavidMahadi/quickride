...
class _CompaniesScreenState extends State<CompaniesScreen>
  ...
  static const _tabs = ['All', 'Daily', 'Monthly', 'Long-Term', 'Hybrid'];
  ...
  String get _selectedModel => _tabs[_tabCtrl.index];

  List<RentalCompany> get _filtered {
    var list = allCompanies.where((c) {
      final matchFilter = _filter.isEmpty ||
          c.name.toLowerCase().contains(_filter) ||
          c.categories.any((cat) => cat.toLowerCase().contains(_filter)) ||
          c.location.toLowerCase().contains(_filter);
      final matchModel = _selectedModel == 'All' || c.rentalModel == _selectedModel;
      return matchFilter && matchModel;
    }).toList();
    return list;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        _RentalModelTabBar(
          controller: _tabCtrl,
          border: border, bg: bg, surface: surface,
        ...
        Padding(
          ...
          child: Row(children: [
            Text('${results.length} ${results.length == 1 ? 'company' : 'companies'}',
                style: TextStyle(fontSize: 12, color: textSec)),
            if (_selectedModel != 'All') ...[
              const SizedBox(width: 6),
              Text('· ${_selectedModel} only',
        ...
        Expanded(
          child: results.isEmpty
              ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text('Try a different filter', style: TextStyle(color: textSec, fontSize: 12)),
                ]))
              : _selectedModel == 'All' && _filter.isEmpty
                  ...
                  // Specific tab or search: flat list
                  : ListView.separated(
                      padding: const EdgeInsets.fromLTRB(20, 4, 20, 24),
                      itemCount: results.length,
                      separatorBuilder: (_, __) => const SizedBox(height: 12),
                      itemBuilder: (_, i) => _CompanyCard(
                        company: results[i], isDark: isDark,
...
// ── Company list card ─────────────────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════
//  RENTAL MODEL TAB BAR
// ═══════════════════════════════════════════════════════════════
class _RentalModelTabBar extends StatelessWidget {
  final TabController controller;
  final Color border, bg, surface, textPri, textSec;
  const _RentalModelTabBar({
    required this.controller, required this.border, required this.bg,
    required this.surface, required this.textPri, required this.textSec,
  });

  static const _tabs = [
    ('All',       Icons.grid_view_rounded,      Color(0xFF8B91A8)),
    ('Daily',     Icons.today_rounded,           Color(0xFF3B5FD4)),
    ('Monthly',   Icons.calendar_month_rounded,  Color(0xFF1D9E75)),
    ('Long-Term', Icons.event_repeat_rounded,    Color(0xFF7F77DD)),
    ('Hybrid',    Icons.swap_horiz_rounded,      Color(0xFFD4A017)),
  ];
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: sel ? color.withOpacity(0.15) : surface,
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(t.$2, color: sel ? color : textSec, size: 14),
                const SizedBox(width: 6),
                Text(t.$1,
                    style: TextStyle(
                      fontSize: 12,
...
// ═══════════════════════════════════════════════════════════════
class _GroupedCompanyList extends StatelessWidget {
  final List<RentalCompany> companies;
  final bool isDark;
  final Color border, textPri, textSec;
  const _GroupedCompanyList({
    required this.companies, required this.isDark,
    required this.border, required this.textPri, required this.textSec,
  });

  static const _groupOrder = ['Daily', 'Monthly', 'Long-Term', 'Hybrid'];

  @override
  Widget build(BuildContext context) {
    // Group companies by rentalModel
    final Map<String, List<RentalCompany>> groups = {};
    for (final model in _groupOrder) {
      final list = companies.where((c) => c.rentalModel == model).toList();
      if (list.isNotEmpty) groups[model] = list;
    }
    // Any unknown model goes to Daily
    final ungrouped = companies.where((c) => !_groupOrder.contains(c.rentalModel)).toList();
    if (ungrouped.isNotEmpty) groups['Daily'] = [...(groups['Daily'] ?? []), ...ungrouped];

    final sections = groups.entries.toList();

    return ListView.builder(
      padding: const EdgeInsets.fromLTRB(20, 4, 20, 24),
      itemCount: sections.fold<int>(0, (sum, e) => sum + e.value.length),
      itemBuilder: (_, idx) {
        int offset = 0;
        for (final entry in sections) {
          if (idx < offset + entry.value.length) {
            final company = entry.value[idx - offset];
            return Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: _CompanyCard(
                company: company, isDark: isDark,
                border: border, textPri: textPri, textSec: textSec,
              ),
            );
          }
          ...
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                ...
                const SizedBox(width: 6),
                _RentalModelBadge(rentalModel: company.rentalModel),
              ]),
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Company link copied!'), backgroundColor: Color(0xFF1D9E75)));
                      },
                    ),
                  ]),
                ),
...
// ── Rental model badge used in company cards ──────────────────
class _RentalModelBadge extends StatelessWidget {
  final String rentalModel;
  const _RentalModelBadge({required this.rentalModel});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 3),
      Text(rentalModel, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: _color)),
    ]),
  );
}
