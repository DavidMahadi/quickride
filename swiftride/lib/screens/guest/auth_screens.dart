...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _submit() async {
    ...
    final err = await AuthService.login(_emailCtrl.text.trim(), _passwordCtrl.text.trim());
    if (!mounted) return;
          ...
          SizedBox(width: double.infinity, height: 52, child: ElevatedButton(
            onPressed: (_agreed && !_loading) ? () async {
              ...
              try {
                ...
                if (err != null) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(err), backgroundColor: const Color(0xFFD85A30)));
                  return;
                }
                ...
                setState(() => _loading = false);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(e.toString()), backgroundColor: const Color(0xFFD85A30)));
              }
            } : null,
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColors.gold, foregroundColor: Colors.black,
