                  ...
                  onTap: () {
                    ...
                    showModalBottomSheet(
                      ...
                      builder: (_) => DraggableScrollableSheet(
                        ...
                        builder: (_, ctrl) {
                          final userBookings = _store.bookings
                              .where((b) => b.customerName == u.name).toList();
                          return Container(
                            decoration: BoxDecoration(
                            ...
                            child: Column(children: [
                              ...
                              Padding(padding: const EdgeInsets.fromLTRB(20, 10, 20, 10),
                                child: Row(children: [
                                  const Icon(Icons.receipt_long_outlined, color: Color(0xFF3B5FD4), size: 18),
                                  const SizedBox(width: 8),
                                  Text('${u.name}\u2019s Bookings',
                                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: textPri)),
                                  const Spacer(),
                                  Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                                    decoration: BoxDecoration(color: const Color(0xFF3B5FD4).withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                                    child: Text('${userBookings.length} total',
                                      style: const TextStyle(fontSize: 11, color: Color(0xFF3B5FD4), fontWeight: FontWeight.w700))),
                                ])),
                              Divider(color: border, height: 1),
                              Expanded(child: userBookings.isEmpty
                                ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                                    Icon(Icons.receipt_long_outlined, size: 48, color: textSec),
                                    const SizedBox(height: 10),
                                    Text('No bookings yet', style: TextStyle(color: textSec, fontSize: 14)),
                                  ]))
                                : ListView.separated(
                                    controller: ctrl,
                                    padding: const EdgeInsets.all(16),
                                    ...
                                    itemBuilder: (_, i) {
                                      ...
                                      final sc = b.status == 'Active' ? const Color(0xFF1D9E75)
                                              : b.status == 'Upcoming' ? const Color(0xFF3B5FD4)
                                              : b.status == 'Completed' ? const Color(0xFF8B91A8)
                                              : const Color(0xFFD85A30);
                                      return Container(
                                        padding: const EdgeInsets.all(14),
                                        decoration: BoxDecoration(
                                          color: isDark ? const Color(0xFF141828) : const Color(0xFFF8F9FC),
                                          borderRadius: BorderRadius.circular(14),
                                          border: Border.all(color: border, width: 0.5)),
                                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                          Row(children: [
                                            ...
                                            const SizedBox(width: 12),
                                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                              Text(b.carName,  style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: textPri)),
                                              Text(b.companyName, style: TextStyle(fontSize: 11, color: textSec)),
                                            ])),
                                            Container(
                                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                                              decoration: BoxDecoration(color: sc.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                                              child: Text(b.status, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: sc))),
                                          ]),
                                          const SizedBox(height: 10),
                                          Row(children: [
                                            ...
                                            Text('${b.from} → ${b.to}', style: TextStyle(fontSize: 11, color: textSec)),
                                            const Spacer(),
                                            Text('\$${b.amount.toInt()}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFFD4A017))),
                                          ]),
                                        ]));
                                    })),
                            ]));
                ...
                _ActionBtn(
                  ...
                  onTap: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text('${u.name} has been $newStatus'),
                      backgroundColor: newStatus == 'Active' ? const Color(0xFF1D9E75) : const Color(0xFFD85A30)));
                  }),
                    ...
                    showDialog(
                      ...
                      builder: (_) {
                        ...
                        return StatefulBuilder(builder: (ctx, ss) => AlertDialog(
                          backgroundColor: isDark ? const Color(0xFF141828) : Colors.white,
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
                          title: Text('Reset Password', style: TextStyle(color: textPri, fontWeight: FontWeight.w800, fontSize: 16)),
                          content: sent
                            ? Column(mainAxisSize: MainAxisSize.min, children: [
                                const Icon(Icons.mark_email_read_outlined, color: Color(0xFF1D9E75), size: 48),
                                const SizedBox(height: 12),
                                Text('Reset link sent to ${u.email}',
                                  textAlign: TextAlign.center,
                                  style: TextStyle(color: textSec, fontSize: 13)),
                              ])
                            : Column(mainAxisSize: MainAxisSize.min, children: [
                                Icon(Icons.lock_reset_rounded, color: const Color(0xFF7F77DD), size: 40),
                                const SizedBox(height: 12),
                          ...
                          actions: sent
                            ? [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Done', style: TextStyle(color: Color(0xFF7F77DD))))]
                            : [
                                TextButton(onPressed: () => Navigator.pop(ctx),
                                  child: Text('Cancel', style: TextStyle(color: textSec))),
                                ElevatedButton(
                                  onPressed: () => ss(() => sent = true),
                                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF7F77DD), foregroundColor: Colors.white,
                                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                    ...
                    showModalBottomSheet(
                      ...
                      builder: (_) => Padding(
                        ...
                        child: Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 0), child: Column(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          const SizedBox(height: 14),
                          Row(children: [
                            const Icon(Icons.mail_outline_rounded, color: Color(0xFFD4A017), size: 18),
                            const SizedBox(width: 8),
                            Text('Message to ${u.name}',
                              style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: textPri)),
                          ]),
                          const SizedBox(height: 16),
                          TextField(
                            ...
                            decoration: InputDecoration(
                              ...
                              filled: true,
                              fillColor: isDark ? const Color(0xFF0A0E1A) : const Color(0xFFF3F4F8),
                          ...
                          SizedBox(width: double.infinity, child: ElevatedButton.icon(
                            onPressed: () {
                              ...
                              Navigator.pop(context);
                              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                content: Text('Message sent to ${u.name}'),
            ...
            Expanded(child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return GestureDetector(
                  onTap: () {
                    Navigator.pop(ctx);
                    // Route each notification to its relevant screen
                    switch (i) {
                      case 0: // New booking confirmed → Bookings tab
                        _tc.animateTo(2); setState(() => _tab = 2);
                        break;
                      case 1: // Company pending approval → Companies tab
                        _tc.animateTo(1); setState(() => _tab = 1);
                        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
                          content: Text('Showing pending companies'),
                          backgroundColor: Color(0xFFD4A017)));
                        break;
                      case 2: // Account suspended → Users tab, open user detail
                        _tc.animateTo(3); setState(() => _tab = 3);
                        WidgetsBinding.instance.addPostFrameCallback((_) {
                          final isDark = Theme.of(ctx).brightness == Brightness.dark;
                          final bg = isDark ? const Color(0xFF141828) : Colors.white;
                          final br = isDark ? const Color(0xFF252B3E) : const Color(0xFFDDE1EE);
                          final tp = isDark ? Colors.white : const Color(0xFF0A0E1A);
                          final ts = isDark ? const Color(0xFF8B91A8) : const Color(0xFF6B7280);
                          final u = _users.firstWhere((u) => u.name.contains('Bob'), orElse: () => _users.first);
                          _showUserDetail(ctx, u, isDark, bg, br, tp, ts);
                        });
                        break;
                      case 3: // New review posted → Companies tab
                        _tc.animateTo(1); setState(() => _tab = 1);
                        break;
                      case 4: // New user registered → Users tab
                        _tc.animateTo(3); setState(() => _tab = 3);
                        break;
                    }
                  },
                    ...
                    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(children: [
                          ...
                          const Spacer(),
                          Icon(Icons.chevron_right_rounded, size: 14, color: textSec.withOpacity(0.5)),
                        ]),
                      ])),
                    ]),
                  ),
