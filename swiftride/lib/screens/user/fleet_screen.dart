...
import 'package:swiftride/screens/user/booking_flow_screen.dart' show BookingFlowScreen;
              ...
              if (_selecting)
                ...
                GestureDetector(
                  onTap: () {
                    ...
                    if (avail.isNotEmpty) _toggle(avail.first);
                  },
                  child: Container(
                    ...
                    child: const Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.check_box_outlined, color: Colors.white, size: 15),
                      SizedBox(width: 4),
                      Text('Select', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
                    ]),
                  ),
                ),
              ],
            ...
            Expanded(child: ListView(controller: ctrl, padding: const EdgeInsets.all(20), children: [
              ...
              SizedBox(width: double.infinity, child: ElevatedButton.icon(
                onPressed: () {
                  ...
                  if (!AuthService.isLoggedIn) {
                    AuthService.setPending('/user/booking-flow', args: {
                      'name': cars.map((c) => c.name).join(', '),
                  ...
                  final multiCar = <String, dynamic>{
                    'id':           cars.map((c) => c.name).join('_'),
                    'name':         cars.length == 1
                                      ? cars.first.name
                                      : '${cars.length} Vehicles (${cars.map((c) => c.name).join(', ')})',
                    'brand':        widget.company.name,
                    'company':      widget.company.name,
                    'price':        total.toInt(),
                    'category':     cars.first.category,
                    'seats':        cars.fold(0, (s, c) => s + c.seats),
                    'fuel':         cars.first.fuel,
                    'transmission': cars.first.transmission,
                    'isMulti':      true,
                    'carCount':     cars.length,
                    'carNames':     cars.map((c) => c.name).toList(),
                  };
                  Navigator.push(context, MaterialPageRoute(
                    builder: (_) => BookingFlowScreen(car: multiCar),
                  ));
                },
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Stack(children: [
              ...
              // Category tag — hidden when selecting mode is active
              if (!isSelecting && !isSelected) Positioned(top: 10, left: 10, child: Container(
                padding: const EdgeInsets.symmetric(horizontal:10, vertical:4),
              ...
              // Selection checkbox — always visible, prominent when selecting
              Positioned(top: 10, left: 10, child: GestureDetector(
                onTap: car.available ? onSelect : null,
                child: AnimatedOpacity(
                  duration: const Duration(milliseconds: 180),
                  opacity: isSelecting || isSelected ? 1.0 : 0.0,
                  child: AnimatedContainer(
                    duration: const Duration(milliseconds: 180),
                    width: 26, height: 26,
                    decoration: BoxDecoration(
                      color: isSelected ? _c : Colors.black.withOpacity(0.35),
                      borderRadius: BorderRadius.circular(6),
                      border: Border.all(
                        color: isSelected ? _c : Colors.white.withOpacity(0.7),
                        width: 1.5)),
                    child: isSelected
                      ? const Icon(Icons.check_rounded, color: Colors.white, size: 16)
                      : const Icon(Icons.check_box_outline_blank_rounded, color: Colors.white, size: 16),
                  ),
                ),
              )),
